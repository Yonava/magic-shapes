import { areBoundingBoxesOverlapping, isPointInLine, normalizeBoundingBox, } from '../../helpers';
/**
 * @param point - the point to check if it is in the line
 * @returns a function that checks if the point is in the line
 */
export const lineHitbox = (schema) => (point) => {
    return isPointInLine(schema, point);
};
export const getLineBoundingBox = (schema) => () => {
...
export const lineEfficientHitbox = (schema) => {
    const { start, end, lineWidth } = schema;
    ...
    return (boxToCheck) => {
        ...
        const inBoundingBox = areBoundingBoxesOverlapping(getLineBoundingBox(schema)(), boxToCheck);
        if (!inBoundingBox)
            return false;
        ...
        return segmentBoundBoxes.some((bb) => areBoundingBoxesOverlapping(bb, boxToCheck));
    };
};
