import { areBoundingBoxesOverlapping, normalizeBoundingBox, rotatePoint, toBorderRadiusArray, } from '../../helpers';
 ...
 */
export const rectHitbox = (schema) => (point) => {
    ...
    const noBorderRadius = (typeof borderRadius === 'number' && borderRadius === 0) ||
        (Array.isArray(borderRadius) && borderRadius.every((r) => r === 0));
    if (noBorderRadius) {
        return (localPoint.x >= x - strokeWidth / 2 &&
    ...
    const [topLeftRadius, topRightRadius, bottomRightRadius, bottomLeftRadius] = radii.map((r) => Math.min(Math.max(r, 0), maxRadius));
    const rectangles = [
        {
            x: x + topLeftRadius,
            y,
...
export const getRectBoundingBox = (schema) => () => {
...
export const rectEfficientHitbox = (schema) => (boxToCheck) => areBoundingBoxesOverlapping(getRectBoundingBox(schema)(), boxToCheck);
