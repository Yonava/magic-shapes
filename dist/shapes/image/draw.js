 ...
 */
const drawMissingMediaCheckerboard = (width, height, ctx) => {
    const squareSize = 10;
    ...
    for (let y = 0; y < Math.ceil(height / squareSize); y++) {
        for (let x = 0; x < Math.ceil(width / squareSize); x++) {
            ...
            ctx.fillRect(startX + x * squareSize, startY + y * squareSize, squareSize, squareSize);
        }
    }
};
export const drawImageWithCtx = (schema) => {
