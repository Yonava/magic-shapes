...
export const drawUTurnWithCtx = (schema) => {
    const { spacing, at, upDistance, downDistance, rotation, lineWidth, fillColor: color, fillGradient, } = schema;
    ...
    if (fillGradient && fillGradient.length >= 2) {
        ...
        const gradientColorAtCircleStart = getColorAtPercentage(fillGradient, upDistance / totalLength);
        const gradientColorAtCircleEnd = getColorAtPercentage(fillGradient, (totalLength - downDistance) / totalLength);
        lineGradient = [
        ...
        circleGradient = [
            ...
            ...fillGradient
                .filter((stop) => stop.offset >= upDistance / totalLength &&
                stop.offset <= (totalLength - downDistance) / totalLength)
                .map((stop) => ({
                offset: (stop.offset - upDistance / totalLength) /
        ...
        arrowGradient = [
            ...
            ...fillGradient
                .filter((stop) => stop.offset >= (totalLength - downDistance) / totalLength)
                .map((stop) => ({
                offset: (stop.offset - (totalLength - downDistance) / totalLength) /
