...
export const useAnimatedShapes = () => {
    ...
    const schemaIdToShapeName = new Map();
    const { defineTimeline, timelineIdToTimeline } = useDefineTimeline({
        ...
        stop: ({ shapeId, timelineId }) => {
            ...
            if (stillRunning.length === 0)
                return activeAnimations.delete(shapeId);
     ...
     */
    const getAnimatedSchema = (schemaId) => {
        ...
        for (const animation of animations) {
            ...
            if (!timeline)
                throw new Error("animation activated without a timeline!");
    ...
    const animatedFactory = (factory, shapeName) => (schema) => new Proxy(factory(schema), {
        get: (target, rawProp) => {
            const prop = rawProp;
            if (!shapeProps.has(prop))
                return target[prop];
            const animations = activeAnimations.get(schema.id);
            const defaultResolver = getSchemaWithDefaults?.[shapeName];
            if (!defaultResolver)
            ...
            if (targetMapSchema)
                return factory(targetMapSchema)[prop];
            if (!animations || animations.length === 0)
                return target[prop];
            if (!animations[0]?.schemaWithDefaults)
                animations[0].schemaWithDefaults = schemaWithDefaults;
            if (prop === "startTextAreaEdit")
                return console.warn("shapes with active animations cannot spawn text inputs");
    ...
    return {
         ...
         * will crash your app!
         */
        getAnimatedProp: (schemaId, inputPropName) => {
            const animations = activeAnimations.get(schemaId);
            if (!animations || animations.length === 0) {
            ...
            let propVal = schemaWithDefaults[inputPropName];
            for (const animation of animations) {
                const timeline = timelineIdToTimeline.get(animation.timelineId);
                if (!timeline)
                    throw new Error("(Internal Error) Animation activated without a timeline!");
                const animationWithTimeline = {
                    ...timeline,
                    ...animation,
                };
                ...
                const { properties } = animationWithTimeline;
                const progress = getAnimationProgress(animationWithTimeline);
                const fn = properties[inputPropName];
