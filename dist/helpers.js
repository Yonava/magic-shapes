 ...
 */
export const rotatePoint = (pointToRotate, centerPoint, angle) => {
 ...
 * @param angle - the angle in radians that the pointToRotate will rotate by
 */
export const rotatePointInPlace = (pointToRotate, centerPoint, angle) => {
    const cosA = Math.cos(angle);
    const sinA = Math.sin(angle);
    const dx = pointToRotate.x - centerPoint.x;
    const dy = pointToRotate.y - centerPoint.y;
 ...
 */
export const getLargestAngularSpace = (center, points) => {
    if (points.length === 0)
        return 0;
    const [firstPoint] = points;
    if (points.length === 1)
        return getAngle(center, firstPoint) + Math.PI;
    const angles = points
 ...
 */
export const getArrowHeadSize = (arrowWidth) => {
 ...
 */
export const calculateArrowHeadCorners = (options) => {
 ...
 */
export const angleDifference = (angleA, angleB) => Math.abs(Math.atan2(Math.sin(angleA - angleB), Math.cos(angleA - angleB)));
/**
 ...
 */
export const getColorAtPercentage = (gradient, percentage) => {
 ...
 */
export const normalizeBoundingBox = (bb) => ({
    at: {
 ...
 */
export const getCenterPoint = (bb) => {
    const { at: { x, y }, width, height, } = bb;
    return {
        x: x + width / 2,
        y: y + height / 2,
    };
 ...
 */
export const areBoundingBoxesOverlapping = (bb1, bb2) => {
    const { at: bb1At, width: bb1Width, height: bb1Height, } = normalizeBoundingBox(bb1);
    const { at: bb2At, width: bb2Width, height: bb2Height, } = normalizeBoundingBox(bb2);
 ...
 */
export const isPointInLine = (line, point) => {
 ...
 */
export const isPointInBoundingBox = (bb, pt) => {
    const { at: { x, y }, width, height, } = normalizeBoundingBox(bb);
    const { x: xPt, y: yPt } = pt;
    const xLegit = xPt >= x && xPt <= x + width;
    const yLegit = yPt >= y && yPt <= y + height;
    return xLegit && yLegit;
};
export const toBorderRadiusArray = (borderRadius) => {
    if (!borderRadius) {
        return [0, 0, 0, 0];
    }
...
const isDashArray = (dash) => Array.isArray(dash);
export const drawStrokeOntoShape = (ctx, stroke) => {
    ...
    if (dash) {
        ...
        ctx.setLineDash((isArr ? dash : dash.pattern));
