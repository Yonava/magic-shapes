import type { ShapeNameToSchema } from '../../types';
...
type TextAreaCustomOption<TSchema> = (propValue: TextArea, schema: TSchema) => TextArea;
type CustomOption<TProp, TSchema> = (propValue: DeepRequired<TProp>, schema: TSchema) => TProp;
type WithCustomOption<TSchema> = {
    [TProp in keyof TSchema]: TProp extends 'textArea' ? TSchema[TProp] | TextAreaCustomOption<TSchema> : TSchema[TProp] | CustomOption<TSchema[TProp], TSchema>;
};
type WithObjectOption<TSchema> = {
    [TProp in keyof TSchema]: TSchema[TProp] | {
        value: TSchema[TProp];
        easing?: EasingOption;
    };
};
type TimelinePropsWithCustomOption<TSchema> = WithObjectOption<WithCustomOption<TSchema>>;
/**
...
export type Timeline<T extends keyof ShapeNameToSchema> = DeepReadonly<{
     ...
     */
    synchronize?: boolean;
} & TimelinePlaybackDuration & TimelinePlaybackDelay & TimelineCustomInterpolations<SchemaWithDefaults[NoInfer<T>]>>;
export declare const useDefineTimeline: (controls: UseDefineTimelineOptions) => {
    timelineIdToTimeline: Map<string, CompiledTimeline>;
    defineTimeline: <T extends keyof ShapeNameToSchema>(timeline: Timeline<T>) => TimelineControls;
};
